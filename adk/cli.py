...
async def _run(args: argparse.Namespace) -> int:
    ...
    try:
        manifest, skills = load_package(args.package_dir)
    ...
    if args.dry_run:
        ...
        print(f"Would register: agent={manifest.agent_id!r}  skills={[s.skill_id for s in skills]}")
        return 0
    ...
    print("\nRegistering package...")
    try:
        reg_result = await register_package(manifest, skills, tenant_id=args.tenant_id)
